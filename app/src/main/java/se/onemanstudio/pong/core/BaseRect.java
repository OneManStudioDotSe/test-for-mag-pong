...
/**
 * Base class for our graphical objects. We keep track of position and scale (size) here.
 * Rather than holding these in fields and copying them to the model/view matrix as needed, we just use the matrix as storage.
 */
public class BaseRect {
    /**
     * Model/view matrix for this object. Updated by setPosition() and setScale(). This should be merged with the projection matrix when it's time to draw the object.
     */
    protected float[] mModelView;

    /**
     * Simple square, specified as a triangle strip. The square is centered on (0,0) and has a size of 1x1. Triangles are 0-1-2 and 2-1-3 (counter-clockwise winding).
     */
    ...
    /**
     * Texture coordinates. These are flipped upside-down to match pixel data that starts at the top left (typical of many image formats).
     */
    ...
    /**
     * Square, suitable for GL_LINE_LOOP. (The standard COORDS will create an hourglass.). This is expected to have the same number of vertices and coords per vertex as COORDS.
     */
