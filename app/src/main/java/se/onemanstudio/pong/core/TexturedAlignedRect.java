...
/**
 * Represents a two-dimensional axis-aligned textured rectangle. Similar to BasicAlignedRect, but we need to manage texture data as well.
 */
public class TexturedAlignedRect extends BaseRect {
    private static final String TAG = "TexturedAlignedRect";
    ...
    /*
     * Scratch storage for the model/view/projection matrix. We don't actually need to retain it between calls, but we also don't want to re-allocate space for it every time we draw this object.
     * Because all of our rendering happens on a single thread, we can make this static instead of per-object. To avoid clashes within a thread, this should only be used in draw().
     */
    ...
    public TexturedAlignedRect() {
        ...
        // Allocate a FloatBuffer to hold our texture coordinate data, and populate it with default values. These may be overwritten by setTextureCoords().
    ...
    /**
     * Specifies the rectangle within the texture map where the texture data is. By default, the entire texture will be used.
     * <p>
     * Texture coordinates use the image coordinate system, i.e. (0,0) is in the top left. Remember that the bottom-right coordinates are exclusive.
     *
    ...
    public void draw() {
        ...
        // In OpenGL ES 1.1 you needed to call glEnable(GLES20.GL_TEXTURE_2D).  This is not required in 2.0, and will actually raise a GL_INVALID_ENUM error.
