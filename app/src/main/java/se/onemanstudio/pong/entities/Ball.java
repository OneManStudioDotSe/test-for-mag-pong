...
public class Ball extends TexturedAlignedRect {
    ...
    private int mSpeed;                          // Speed, expressed in terms of steps per second. A speed of 60 will move the ball 60 arena-units per second, or 1 unit per frame on a 60Hz device.


    @SuppressWarnings("unused")
    public Ball() {
        if (true) {//TODO: try with false
    ...
    private ByteBuffer generateBallTexture() {
        /*
         * Most images used in games are generated with external tools and then loaded from image files. This is an example of generating texture data directly.
         * We "render" it into a byte[], then copy that into a direct byte buffer.
         */
        byte[] buf = new byte[TEX_SIZE * TEX_SIZE * BYTES_PER_PIXEL];

        /*
         * We're drawing a filled circle with a radius of 31, which gives us a circle that fills a 63x63 area.
         * We're using a 64x64 texture, so we "Stretch" the circle slightly when generating the data, doubling-up the center row and column, to fill the circle to 64x64. Should look fine.
         */
